...
export class AnalysisCache {
  ...
  getChangedFiles(
    ...
    for (const file of currentFiles) {
      ...
      if (!oldMeta) {
        newFiles.push(file);
      } else if (oldMeta.hash !== currentMeta.hash || oldMeta.mtime !== currentMeta.mtime) {
        changed.push(file);
      } else {
        unchanged.push(file);
      }
...
export class CodeFixer {
  ...
  async fix(result: ScanResult, options: {
    backup?: boolean;
    interactive?: boolean;
  }): Promise<FixResult> {
    const fixed: string[] = [];
    ...
    for (const unusedImport of result.unusedImports) {
      try {
        ...
        fixed.push(`Removed unused import: ${unusedImport.importPath} from ${unusedImport.file}`);
      } catch (error) {
    ...
    for (const unusedExport of result.unusedExports) {
      try {
        ...
        fixed.push(`Commented unused export: ${unusedExport.exportName} in ${unusedExport.file}`);
      } catch (error) {
...
*This guide provides concrete implementation examples for the highest-priority optimizations and features.*
